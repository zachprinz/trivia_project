...
module.exports = {
  ...
  listen(socket) {
    ...
    // 'roomJoined' message, and a room object passed in as data.
    ...
    // Use the emitter declared in the player class to listen for events with a
    // 'roomFailed' message, and a data object passed in as data.
    ...
    // Use the emitter declared in the player class to listen for events with a
    // 'answerGraded' message, and a answer object passed in as data.
    ...
    // Use the emitter declared in the player class to listen for events with a
    // 'roundBegin' message, and a data object passed in as data.
    player.emitter.on('roundBegin', (data) => {
      // Emit a message with a roundBeing message, a JSON representing the
      // question, and the data objeect
      socket.emit('roundBegin', Object.assign(player.getRoom().getCurrentQuestion().toJSON(), data));
    });

    // Use the emitter declared in the player class to listen for events with
    // a 'roundEnd' message, and a time object passed in as data.
    player.emitter.on('roundEnd', (midRoundTime) => {
      // Emit a message with the same roundEnd message and a time JSON
      socket.emit('roundEnd', { time: midRoundTime });
    });
    ...
    socket.on('startGame', (data) => {
      if (player.getRoom().getAdmin().getID() === player.getID()) {
        player.getRoom().setNumRounds(parseInt(data.numRounds, 10));
        player.getRoom().setRoundTime(parseInt(data.roundTime, 10));
        player.getRoom().beginGame();
      }
    });
